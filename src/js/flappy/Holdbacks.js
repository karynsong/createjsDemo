...
var Holdbacks = ns.Holdbacks = Hilo.Class.create({
    ...
    getSafePosition(bird){
        ...
        for(var i = 0, len = this.children.length; i < len; i++){
            ...
            var absDistance = Math.abs(this.children[i].x + this.x - bird.x);
            if(absDistance < minX){
                minX = absDistance;
                minIndex = i;
            }
        ...
        if(minIndex !== -1){
            var downHose = this.children[minIndex],
                upHose = this.children[minIndex + 1],
                safeMinY = upHose.height + upHose.y,
                safeMaxY = downHose.y;
        }
        return {
            safeMinY,
